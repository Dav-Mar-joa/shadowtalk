...
export const AVATAR_IMAGES = Array.from({ length: 24 }, (_, i) => ({
  id: `avatar${i + 1}`,
  src: `/profil_face/avatar${i + 1}.png`,
  label: `Face ${i + 1}`
}));
...
export function getAvatarDisplay(avatarId, avatarImage) {
  // 🔥 cas image upload custom (déjà existant)
  if (avatarId === 'custom' && avatarImage) {
    return { type: 'image', value: avatarImage };
  }

  // 🖼️ cas avatars images (avatar1 → avatar24)
  const img = AVATAR_IMAGES.find(a => a.id === avatarId);
  if (img) {
    return { type: 'image', value: img.src };
  }
