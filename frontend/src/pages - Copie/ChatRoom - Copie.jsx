...
import { get, post, encrypt, decrypt, formatTime, apiDirect } from '../utils/api';
...
export default function ChatRoom() {
  ...
  useEffect(() => {
    ...
    socket.on('new_message', msg => {
      ...
      apiDirect(`/messages/${idRef.current}/read`, { method: 'POST' }).catch(() => {});
    });
  ...
  async function deleteMessage(msgId) {
    setMenuMsg(null);
    await apiDirect(`/messages/${msgId}`, { method: 'DELETE' });
  }

  async function react(msgId, emoji) {
    setReactMenu(null);
    await apiDirect(`/messages/${msgId}/react`, { method: 'POST', body: JSON.stringify({ emoji }) });
  }

  async function deleteChat() {
    if (!confirm('Supprimer ce chat et tous les messages ?')) return;
    await apiDirect(`/chats/${id}`, { method: 'DELETE' });
  }
