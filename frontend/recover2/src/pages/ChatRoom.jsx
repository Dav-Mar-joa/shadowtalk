...
import { get, encrypt, decrypt, formatTime } from '../utils/api';
...
export default function ChatRoom() {
  ...
  function sendText(e) {
    ...
    setMessages(prev => [...prev, {
      ...
      encryptedContent: encrypt(input.trim()),
      createdAt: new Date().toISOString(), readBy: [user._id]
    }]);
    socket.emit('send_message', { chatId: id, type: 'text', encryptedContent: encrypt(input.trim()), tempId });
    setInput('');
  ...
  function isRead(msg) {
    return (msg.readBy||[]).some(r => (r._id||r) !== user._id);
  }
    ...
    <div className="chatroom">
      ...
      <div className="chatroom-header">
        ...
        <div style={{position:'relative',flexShrink:0}}>
          ...
          {!chat?.isGroup && other && <span style={{position:'absolute',bottom:0,right:0}}><OnlineDot online={isOnline(other._id)} size="sm"/></span>}
        </div>
      ...
      <div className="chatroom-messages" onClick={() => { setMenuMsg(null); setReactMenu(null); }}>
        ...
        {messages.map((msg, i) => {
          ...
          const reactions = msg.reactions ? (msg.reactions instanceof Map ? Object.fromEntries(msg.reactions) : msg.reactions) : {};
                ...
                <div className="bubble-wrap">
                  ...
                  {hasReact && (
                    <div className={`msg-reactions ${self?'reactions-self':'reactions-other'}`}>
                      {Object.entries(reactions).map(([emoji, users]) =>
                        users.length > 0 && (
                          <span key={emoji} className={`reaction-chip ${(users||[]).includes(user._id)?'mine':''}`}
                  ...
                  {reactOpen && (
                    <div className={`reaction-picker ${self?'picker-self':'picker-other'}`}
                      onClick={e => e.stopPropagation()}>
                      {REACTIONS.map(e => (
                        <button key={e} className="reaction-pick-btn" onClick={() => react(msg._id, e)}>{e}</button>
                      ))}
                    </div>
                  )}
                </div>
      ...
      {recording && (
        <div className="recording-bar">
          ...
          <span style={{flex:1,color:'var(--text-3)',fontSize:12}}>Enregistrement en cours...</span>
          <button className="btn-ghost" style={{padding:'6px 14px',fontSize:12,color:'var(--danger)',borderColor:'rgba(255,68,102,.3)'}}
            onClick={() => stopRecording(false)}>✕ Annuler</button>
          <button className="btn-primary btn" style={{padding:'6px 14px',fontSize:12,width:'auto'}}
      ...
      {!recording && (
        <form className="chatroom-input-area" onSubmit={sendText}>
          ...
          <div className="input-wrap">
            <button type="button" className="btn-icon input-action" onClick={() => fileRef.current?.click()} title="Envoyer une image">
            ...
            <button type="button" className="btn-icon input-action" onClick={startRecording} title="Message vocal">
