...
// Expose io + online map aux routes
app.set('io',     io);
...
io.on('connection', socket => {
  ...
  // Envoi de message
  socket.on('send_message', async ({ chatId, encryptedContent, tempId }) => {
    try {
      ...
      const msg  = await Message.create({ chat: chatId, sender: uid, encryptedContent, tempId });
      ...
      socket.emit('msg_error', { tempId, error: e.message });
    }
  });
